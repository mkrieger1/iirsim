...
def from_real(x, N, M=0):
    ...
    >>> [from_real(x, 8) for x in [0.7, 0.8, 0.9, 1.0, 1.1]] # 1.0 -> 128
    [90, 102, 115, 128, 141]

    >>> list(from_real(numpy.linspace(0.6, 1.1, 10), 7)) # 1.0 -> 64
    [38, 42, 46, 49, 53, 56, 60, 63, 67, 70]

    >>> list(from_real(numpy.linspace(0.3, 0.6, 10), 8, -1)) # 0.5 -> 128
    [77, 85, 94, 102, 111, 119, 128, 137, 145, 154]

    >>> list(from_real(numpy.linspace(-4.5, -3.0, 10), 8, 2)) # 4.0 -> 128
...
def to_real(x, N, M=0):
    ...
    >>> [round(to_real(x, 8), 2) for x in [100, 110, 120, 130]] # 128 -> 1.0
    [0.78, 0.86, 0.94, 1.02]

    >>> [round(x, 2) for x in to_real(numpy.arange(48, 73, 4), 7)] # 64 -> 1.0
    [0.75, 0.81, 0.88, 0.94, 1.0, 1.06, 1.13]

    >>> [round(x, 2) for x in to_real(numpy.arange(5, 10), 4, 1)] # 8 -> 2.0
